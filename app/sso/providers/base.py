...
class SSOProvider:
    ...
    async def get_login_url(
        ...
        *,
        ...
        return request_uri  # type: ignore[no-any-return]
    ...
    async def verify_and_process(
        ...
        *,
        ...
        """Processes the login given a SSOCallbackData object. This should be used for the /callback path.

        Args:
            callback (SSOCallback): The callback data containing the code and state.
        ...
        if self.uses_pkce:
            pkce_code_verifier = callback.pkce_code_verifier
    ...
    async def process_login(
        ...
        *,
        ...
        if not self.allow_insecure_http and url.scheme != "https":
            current_url = str(url).replace("http://", "https://")  # noqa
        else:
            current_url = str(url)

        current_path = f"{url.scheme}://{url.host}{url.path}"
